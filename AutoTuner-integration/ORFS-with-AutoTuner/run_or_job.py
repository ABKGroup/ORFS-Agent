...
def extract_ppa_remote(log_dir: str, clk: float, server: str, job_id:int = 0) -> Tuple[float, float, float, float, float, float, float]:
    ...
    print(f"[INFO] Job id:{job_id} Running remote command: {command}")
    ...
    if result.returncode == 0:
        ...
        print(f"[ERROR] Job id:{job_id} Remote RPT Error: {result.stderr}")
...
def run_job(design:str, clk_period:float, util:float, ar:float, tech:str,
            ...
            power_effort:int, gp_pad:int, dp_pad:int, enable_dpo:int,
            cts_cluster_size:int, cts_cluster_dia:int, gp_rd:int, gp_td:int,
            pin_adj:float, up_adj:float, run_dir:str, server:str, run_id:int,
            cached_netlist:Optional[str] = None,
            timeout:int = 10800) -> Tuple[float, float, float, float, float, float]:
    ref_dir=os.path.dirname(os.path.abspath(__file__))
    ...
    print(f"[INFO] Job Id:{run_id} Running: {shell_command}")
    ...
    try:
        result = subprocess.run(shell_command, timeout=timeout, shell=True,
                       check=True, capture_output=True, text=True)
        if result.stderr:
            print(f"[ERROR] Job Id:{run_id} stderr: {result.stderr}")
        ...
        return extract_ppa_remote(f"{run_dir}/logs/{tech}/{design_name}/{job_name}",
                                clk_period, server, run_id)
    except subprocess.TimeoutExpired:
        print(f"[Timeout] Job Id:{run_id} {timeout} seconds")
        return extract_ppa_remote(f"{run_dir}/logs/{tech}/{design_name}/{job_name}",
                                clk_period, server, run_id)
    except subprocess.CalledProcessError as error_message:
        print(f"[ERROR] Job Id:{run_id} Error message: {error_message}")
        return extract_ppa_remote(f"{run_dir}/logs/{tech}/{design_name}/{job_name}",
                                clk_period, server, run_id)
